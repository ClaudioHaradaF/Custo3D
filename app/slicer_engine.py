...
class GCodeWriter:
    ...
    def write(self, layers, path):
        lines = []
        c = self.c
        fh = min(c['layer_height'] * 1.2, 0.35)
        ...
        lines.append(f'; Support: {"enabled (but not generated)" if c["support_enabled"] else "disabled"}')
        lines.append('M73 P0')
        lines.append('G21 ; mm')
        ...
        for layer_idx, (layer_z, layer_paths) in enumerate(layers):
            ...
            for path_type, pts in layer_paths:
                ...
                if path_type == 'travel':
                    ...
                    spd = c['travel_speed']
                    lines.append(f'G0 X{pts[0].x:.3f} Y{pts[0].y:.3f} F{spd:.0f}')
                    for p in pts[1:]:
            ...
            has_printed = True
            lines.append(f'G1 Z{layer_z + zh:.3f}')
        ...
        with open(path, 'w', encoding='utf-8') as f:
            f.write('\n'.join(lines))
        return len(lines), total_e, total_time
...
class BuiltinSlicer:
    ...
    def slice(self, input_path, output_path=None, plate_id=1, **kwargs):
        ...
        total_lines, total_e, total_time = writer.write(gcode_layers, output_path)
        ...
        if total_e > 0:
            path_weight = total_e * cs_area * filament_density / 1000
            weight = min(path_weight, weight)  # path volume should never exceed mesh volume

        elapsed = time.time() - self._start_time

        # Add time for travel moves, retractions, layer changes
        total_time += actual_layers_processed * 5  # ~5s per layer change
        total_time *= 1.15  # 15% margin for acceleration/deceleration
